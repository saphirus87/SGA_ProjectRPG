...
struct VS_OUTPUT
{
    ...
    float3 oViewPos : TEXCOORD7;
};

VS_OUTPUT NormalMapping_Pass_0_Vertex_Shader_vs_main(
    float4 mPosition : POSITION,
    float3 mNormal : NORMAL,
    float3 mBinormal : BINORMAL,
    float3 mTangent : TANGENT,
...
{
    VS_OUTPUT Output;
    Output.oViewPos = mul(mPosition, gWorldMatrix);
    Output.oViewPos = mul(float4(Output.oViewPos, 1.0f), gViewMatrix);

    Output.mPosition = mul(mPosition, gWVPMatrix);
    
    Output.mUV = mUV;
    Output.oNormal = mul(mNormal, (float3x3)gWorldMatrix);
    Output.oNormal.r *= -1.0f;
    Output.oNormal.g *= -1.0f;
    Output.oNormal.b *= -1.0f;
    // v is world position
    float4 worldPos = mul(mPosition, gWorldMatrix);
    ...
    // T B N on World Space
    Output.T = normalize(mul(mTangent, (float3x3) gWorldMatrix));
    Output.B = normalize(mul(mBinormal, (float3x3) gWorldMatrix));
    Output.N = normalize(mul(mNormal, (float3x3) gWorldMatrix));
...
void NormalMapping_Pass_0_Pixel_Shader_ps_main(
    float2 mUV : TEXCOORD0,
    float3 mLightDir : TEXCOORD1,
    float3 mViewDir : TEXCOORD2,
    float3 T : TEXCOORD3,
...
{
    float3 tangentNormal = tex2D(NormalSampler, mUV).rgb; // equal xyz
    tangentNormal = normalize(tangentNormal * 2 - 1); // 0~1 -> -1~1
    ...
    float4 albedo = tex2D(DiffuseSampler, mUV);
    float3 lightDir = normalize(mLightDir);
   
    float3 diffuse = saturate(dot(worldNormal, -lightDir));
       // 범프맵 없는 애들 Normal값
    oNormal = float4(normalize(Normal), 1);
    oPos = float4(Pos, 1);
    float3 specular = 0;
    ...
    {
        float3 reflection = reflect(lightDir, worldNormal);
        float3 viewDir = normalize(mViewDir);
        specular = saturate(dot(reflection, -viewDir));
        specular = pow(specular, 20.0f);
    }
    diffuse = gLightColor * albedo.rgb * diffuse;
    ...
    oCol = float4(ambient + diffuse + specular, 1);
