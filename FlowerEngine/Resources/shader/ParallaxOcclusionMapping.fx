                         ...
                         float3 vInTangentOS : TANGENT)
{
    ...
    Out.Pos = mul(inPositionOS, g_mWorld);
    Out.Pos = mul(float4(Out.Pos, 1.0f), g_mView);
...
//--------------------------------------------------------------------------------------
ComputeOUTPUT ComputeIllumination(float2 texCoord, float3 vLightTS, float3 vViewTS, float fOcclusionShadow)
{
    ComputeOUTPUT output;
   // Sample the normal from the normal map for the given texture sample:
    
    float3 normal = tex2D(tNormalHeightMap, texCoord) * 2 - 1;
    float3 vNormalTS = normalize(normal);
    output.Normal = float4(vNormalTS, 1);
   ...
   // Composite the final color:
    ...
    output.cFinalColor = cFinalColor;
   // float3 vNormal = tex2D(tNormalHeightMap, texCoord) * 2 - 1;
...
PS_OUTPUT RenderScenePS(PS_INPUT i)
{
   ...
   // Compute resulting color for the pixel:
    ComputeOUTPUT output = ComputeIllumination(texSample, vLightTS, vViewTS, fOcclusionShadow);

    cResultColor = output.cFinalColor;
   ...
   // But since this example isn't doing that, we just output the computed result color here:
    PS_OUTPUT OutPS;
    OutPS.RGBColor = cResultColor;
    //OutPS.Normal = float4(-vNormalWS, 1);
    
    OutPS.Normal = output.Normal;
    OutPS.Pos = float4(i.Pos, 1);
...
PS_OUTPUT RenderSceneBumpMapPS(PS_INPUT i)
{
   ...
   // Compute resulting color for the pixel:
    ComputeOUTPUT output = ComputeIllumination(texSample, vLightTS, vViewTS, 1.0f);
    cResultColor = output.cFinalColor;
   ...
   // But since this example isn't doing that, we just output the computed result color here:
    PS_OUTPUT OutPS;
    OutPS.RGBColor = cResultColor;
    //OutPS.Normal = float4(-normalize(i.vNormalWS), 1);
    OutPS.Normal = output.Normal;
    OutPS.Pos = float4(i.Pos, 1);
    return OutPS;
}
